...
class Drawing(val initScreen: Screen) {
  ...
  private def drawLine(line: Line): Content = drawingByJoinPoints(line.point1, line.point2)

  private def drawRectangle(rec: Rectangle): Content = {
  ...
  private def drawingByJoinPoints(point: Point*): Content = {
    val canvas: Canvas =
        if (screen.content.isEmpty) throw new IllegalStateException("Canvas is not created")
        else screen.content.split("\n").map(_.toCharArray)

    val maxX: Int = canvas(0).length - 2
    val maxY: Int = canvas.length - 2

    def safeX(x: Int): Int = safeInt(x, maxX)

    def safeY(y: Int): Int = safeInt(y, maxY)

    def xInsideCanvas(x: Int): Boolean = insideCanvas(x, maxX)

    def yInsideCanvas(x: Int): Boolean = insideCanvas(x, maxY)

    def checkAnyPointInsideCanvas(point: Point*): Unit = {
